...
func sssp_func(iterations int, W [][]int, inlinks []int, outlinks []int,
	mystart, myend int, channel_in, channel_out chan int, 
	sigchannel_in, sigchannel_out chan bool, tid, num_nodes int){

	// randGen := rand.New(rand.NewSource(time.Now().UnixNano()))
	distance := make([]int, num_nodes)
	...
	for myiteration := 0; myiteration < iterations; myiteration++{
		...
		for i := mystart; i < myend; i++ {
			DynSendInt(sigchannel_out, channel_out, distance[i], 0.00001) 
		}
	}
}
...
func main() {
	...
	num_threads := 4
	channels_main_threads := make([]chan int, num_threads)
	channels_threads_main := make([]chan int, num_threads)
	
	sigchannels_in := make([]chan bool, num_threads)
	sigchannels_out := make([]chan bool, num_threads)	
	
	for i := range channels_main_threads {
		channels_main_threads[i] = make(chan int, 100)
		channels_threads_main[i] = make(chan int, 100)
