...
func scale_kernel(i float64, j float64, src []DynRelyInt, s_height int, s_width int) DynRelyInt {
  ...
  lr_w := (1.0-u_w)*(1.0-l_w)
	...
	temp_0 := MultConstRelyIF(ul, ul_w)
	temp_1 := MultConstRelyIF(ur, ur_w)
	temp_2 := MultConstRelyIF(ll, ll_w)
	temp_3 := MultConstRelyIF(lr, lr_w)

	temp_4 := AddIntRely(temp_0, temp_1)
	temp_5 := AddIntRely(temp_4, temp_2)
	temp_6 := AddIntRely(temp_5, temp_3)

	return temp_6
}

func scale_tile(f float64, s_width int, s_height int, ts_height int, te_height int,
	d_width int, channel_in chan DynRelyInt, channel_out chan []DynRelyInt, 
	signalchannel_in, signalchannel_out chan bool, size int) {

	src := make([]DynRelyInt, size)
	for i:=0; i<size; i++ {
		src[i] = <- channel_in
	}
	
  delta := 1.0/f
  dest := make([]DynRelyInt, d_width*(te_height-ts_height))
  ...
  for i := ts_height; i < te_height; i++ {
	...
	DynSendDynIntArray(signalchannel_out, channel_out, dest, 0.00001)
...
func main() {
  ...
  src_tmp, s_width, s_height, err := ReadPpmFile(iFile)
	src := make([]DynRelyInt, len(src_tmp))
  ...
  dest := make([]DynRelyInt, d_height*d_width)

	channels_main_threads := make([]chan DynRelyInt, numThreads)
  for i := range channels_main_threads {
    channels_main_threads[i] = make(chan DynRelyInt, 10)
  }
  channels_threads_main := make([]chan []DynRelyInt, numThreads)
  for i := range channels_threads_main {
    channels_threads_main[i] = make(chan []DynRelyInt, 10)
  }
  ...
  WritePpmFile(dest_int,d_width,d_height,oFile)
}
