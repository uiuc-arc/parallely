...
func pagerank_func(iterations int, W [][]int, inlinks []int, outlinks []int,
	myfirstnode, mylastnode int, inchannel, outchannel chan DynRelyFloat,
  ...
  r := DynRelyFloat{0.15, 1, 1}
  d := DynRelyFloat{0.85, 1, 1}

	pageranks := make([]DynRelyFloat, size)
  ...
  for myiteration := 0; myiteration < iterations; myiteration++{
    ...
    mypageranks := make([]DynRelyFloat, mylastnode-myfirstnode)
    ...
    for node := range mypageranks{
      ...
      for k := 0; k<inlinks[myfirstnode+node]; k++ {
        neighbor := W[myfirstnode+node][k]
				...
				temp0 := DynRelyFloat{float64(outlinks[neighbor]), 1, 1}
				temp1 := MultFloatRely(d, pageranks[neighbor])
				temp2 := DivFloatRely(temp1, temp0)
				
        mypageranks[node] = AddFloatRely(temp2, mypageranks[node])
      }
    }

		for node := range mypageranks{
			DynSendDynFloat(datasigchannel_out, outchannel, mypageranks[node], 0.00001)
		}
  }
}
...
func main() {
  ...
  pagerank := make([]DynRelyFloat, num_nodes)

  for i := range W{
    W[i] = make([]int, num_nodes)
    inlinks[i] = 0
    outlinks[i] = 0
    pagerank[i] = DynRelyFloat{0.15, 1, 1}
  }
  ...
  channels_main_threads := make([]chan DynRelyFloat, num_threads)
  for i := range channels_main_threads {
    channels_main_threads[i] = make(chan DynRelyFloat, 1)
  }
  channels_threads_main := make([]chan DynRelyFloat, num_threads)
  for i := range channels_threads_main {
    channels_threads_main[i] = make(chan DynRelyFloat, 1)
  }
