...
func func_0(tid parallely.Process) {
	...
	for iter := 0; iter < 10; iter++ /*maxiterations=10*/ {
		for _, q := range Q {
			parallely.SendFloat64Array(pageranks[:], 0, q);
		...
		for _, q := range Q {
			...
			j = 0
			parallely.CondreceiveFloat64Array(&atemp, slice[:], 0, q);
...
func func_Q(q parallely.Process) {
	...
	for iter := 0; iter < 10; iter++ /*maxiterations=10*/ {
		...
		atemp = parallely.Randchoice(float32(0.99), 1, 0);

		parallely.CondsendFloat64Array(atemp, newPagerank[:], q, 0);
