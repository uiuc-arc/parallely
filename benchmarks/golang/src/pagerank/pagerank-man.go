...
func func_0(tid parallely.Process) {
	...
	for _, q := range Q {
		...
		temp = Num_threads - 1
		lastthread = parallely.ConvBool(i == temp)
		if lastthread != 0 {
			myend = Num_nodes
		}
		parallely.SendInt(mystart, tid, q)
		parallely.SendInt(myend, tid, q)
		i = i + 1
	}

	for iter = 0; iter < 10; iter++ /*maxiterations=10*/ {
		for _, q := range Q {
			parallely.SendFloat64Array(pageranks[:], tid, q)
		}
		i = 0
		for _, q := range Q {
			mystart = i * NodesPerThread
			...
			j = 0
			parallely.ReceiveFloat64Array(slice[:], tid, q)
...
func func_Q(q parallely.Process) {
	...
	parallely.ReceiveInt(&mystart, q, 0)
	parallely.ReceiveInt(&myend, q, 0)

	for iter = 0; iter < 10; iter++ /*maxiterations=10*/ {
		parallely.ReceiveFloat64Array(pageranks[:], q, 0)
		mysize = myend - mystart
		...
		parallely.SendFloat64Array(newPagerank[:], q, 0)
	}
}
