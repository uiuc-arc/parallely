...
query_str = "go run sssp-noisy.go ../inputs/p2p-Gnutella31.txt 62586 out-exact.txt"
query_str2 = "go run sssp-noisy-off.go ../inputs/p2p-Gnutella31.txt 62586 out-approx.txt"
...
for i in range(10):
    ...
    for i, element in enumerate(approx_results):
        # different maxes?
        if exact_results[i] > 100000 and approx_results[i] - exact_results[i] != 0:
            diffs += 1
