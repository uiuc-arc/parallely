...
func func_0() {
  ...
  var DynMap [6]dieseldist.ProbInterval;
...
for _, q := range(Q) {
 dieseldist.ReceiveFloat64(&workerAlpha, 0, q);
my_chan_index = q * dieseldist.Numprocesses + 0;
__temp_rec_val_1 := dieseldist.GetDynValue(my_chan_index);
DynMap[0] = __temp_rec_val_1;
dieseldist.ReceiveFloat64(&workerBeta, 0, q);
my_chan_index = q * dieseldist.Numprocesses + 0;
__temp_rec_val_2 := dieseldist.GetDynValue(my_chan_index);
DynMap[1] = __temp_rec_val_2;
dieseldist.ReceiveInt(&workerSamples, 0, q);
...
func func_Q(tid int) {
  ...
  var DynMap [10]dieseldist.ProbInterval;
  var my_chan_index int;
  _ = my_chan_index;
  _ = DynMap;
  q := tid;
...
var tempDF1 float64;
DynMap[9] = dieseldist.ProbInterval{1, 0};
DynMap[2] = dieseldist.ProbInterval{1, 0};
mX = 0.0;
DynMap[3] = dieseldist.ProbInterval{1, 0};
mY = 0.0;
...
x=tempF;
DynMap[0] = dieseldist.ProbInterval{0.99, 0.001};
_temp_index_2 := ((q-1)*WorkPerThread)+idx;
tempF=Y[_temp_index_2];
y=tempF;
DynMap[1] = dieseldist.ProbInterval{0.99, 0.001};
...
alpha = mY-tempDF0;
dieseldist.SendFloat64(alpha, tid, 0);
dieseldist.SendDynVal(DynMap[6], tid, 0);
dieseldist.SendFloat64(beta, tid, 0);
dieseldist.SendDynVal(DynMap[7], tid, 0);
dieseldist.SendInt(count, tid, 0);
