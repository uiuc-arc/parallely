...
func SendDynIntArrayO1(value []int, sender, receiver int, DynMap []float64, start int) {
	...
	DynamicChannelMap[my_chan_index] <- maxd
}
...
func SendDynFloat64ArrayCustom(value []float64, sender, receiver int, trackedval float64, start int) {
	my_chan_index := sender*Numprocesses + receiver
...
func ReceiveDynVal(rec_var *float64, receiver, sender int) {
	my_chan_index := sender*Numprocesses + receiver
