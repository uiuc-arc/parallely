...
func func_0() {
	...
	for __temp_0 := 0; __temp_0 < 10; __temp_0++ {
		for _, q := range Q {
			dieseldistrel.SendDynIntArray(distance[:], 0, q, DynMap[:], 0)
		}
		i = 0
		for _, q := range Q {
			mystart = i * NodesPerThread
			...
			dieseldistrel.NoisyReceiveDynIntArray(slice[:], 0, q, DynMap[:], 36682)
...
func func_Q(tid int) {
	...
	for __temp_2 := 0; __temp_2 < 10; __temp_2++ {
		dieseldistrel.NoisyReceiveDynIntArray(distances[:], tid, 0, DynMap[:], 0)
		mysize = myend - mystart
		i = 0
		for __temp_3 := 0; __temp_3 < mysize; __temp_3++ {
			...
			for __temp_4 := 0; __temp_4 < nodeInlinks; __temp_4++ {
				...
				DynMap[46683] = DynMap[46684]
				condition = dieseldistrel.ConvBool(temp == 1)
				DynMap[46685] = 1.0
				temp1 = 1
				DynMap[46686] = 1.0
				temp2 = 2
				temp_bool_5 := condition
				if temp_bool_5 != 0 {
					distance = temp1
				} else {
					distance = temp2
				...
				if temp_bool_5 != 0 {
					DynMap[36682] = DynMap[46683] * DynMap[46685]
				} else {
					DynMap[36682] = DynMap[46683] * DynMap[46686]
				}
				inlink = inlink + 1
			}
			DynMap[46684] = DynMap[36682]
			temp = distance
			_temp_index_6 := i
		...
		dieseldistrel.SendDynIntArray(newDistance[:], tid, 0, DynMap[:], 36683)
	}
