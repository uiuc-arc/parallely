...
func func_0() {
  ...
  var workerAlpha float64;
	...
	for _, q := range(Q) {
		...
		DynMap[4].Delta = math.Abs(tempF) * DynMap[0].Delta;
...
func func_Q(tid int) {
  ...
  q := tid;
	...
	for __temp_2 := 0; __temp_2 < WorkPerThread; __temp_2++ {
		...
		// DynMap[8].Reliability = 0.99 + DynMap[2].Reliability - 1.0;
		// DynMap[8].Delta = DynMap[2].Delta + 0.001;
		tempDF0 = mX+x;
		DynMap[2].Reliability = 0.99 + DynMap[2].Reliability - 1.0;
		DynMap[2].Delta = DynMap[2].Delta + 0.001;
		mX = tempDF0;
		// DynMap[8].Reliability = 0.99 + DynMap[3].Reliability - 1.0;
		// DynMap[8].Delta = 0 + 0.99;
		tempDF0 = mY+y;
		DynMap[3].Reliability = 0.99 + DynMap[3].Reliability - 1.0;
		DynMap[3].Delta = 0 + 0.99;
		mY = tempDF0;
		// DynMap[8].Reliability = 0.99 + 0.99 - 1.0;
		// DynMap[8].Delta = math.Abs(x) * 0.001 + math.Abs(y) * 0.001 + 0.001*0.001;
		tempDF0 = x*y;
		DynMap[9].Reliability = 0.99 + 0.99 - 1.0 + DynMap[4].Reliability - 1.0;
		DynMap[9].Delta = DynMap[4].Delta + (math.Abs(x) * 0.001 + math.Abs(y) * 0.001 + 0.001*0.001);
		tempDF1 = ssXY+tempDF0;
		DynMap[4].Reliability = DynMap[9].Reliability;
		DynMap[4].Delta = DynMap[9].Delta;
		ssXY = tempDF1;
		DynMap[8].Reliability = 0.99;
		DynMap[8].Delta = math.Abs(x) * 0.001 + math.Abs(x) * 0.001 + 0.001 * 0.001;
		tempDF0 = x*x;
		DynMap[9].Reliability = DynMap[5].Reliability + 0.99 - 1.0;
		DynMap[9].Delta = DynMap[5].Delta + (math.Abs(x) * 0.001 + math.Abs(x) * 0.001 + 0.001 * 0.001);
	...
	DynMap[9].Delta = math.Abs(tempF) * DynMap[8].Delta;
...
func main() {
  ...
  for i := 0; i < totalWork; i++ {
    X[i] = rand.NormFloat64()*math.Abs(100)
