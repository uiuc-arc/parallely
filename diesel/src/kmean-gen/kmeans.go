...
for _, q := range(Q) {
 diesel.SendFloat64Array(datatemp[:], 0, q);
diesel.SendFloat64Array(datahumid[:], 0, q);
 }
for __temp_2 := 0; __temp_2 < Iterations; __temp_2++ {
 for _, q := range(Q) {
 diesel.SendFloat64Array(centersTemp[:], 0, q);
diesel.SendFloat64Array(centersHumid[:], 0, q);
 }
temp0 = 0.0;
i = 0;
...
tempi=k;
_ = 1.0;_ = 0.0;
temp_bool_7:= condition; if temp_bool_7 != 0 { mincenter  = tempi } else { mincenter = mincenter };
k = k+1;
 }
_temp_index_8 := i;
