...
for _, q := range(Q) {
 dieselacc.SendDynFloat32ArrayO1(input32[:], 0, q, DynMap[:], 10002);
 }
for _, q := range(Q) {
 dieselacc.ReceiveDynFloat32ArrayO1(slice[:], 0, q, DynMap[:], 20002);
i = 0;
...
var temp32 float32;
DynMap[22009] = 0.0;
dieselacc.ReceiveDynFloat32ArrayO1(array32[:], tid, 0, DynMap[:], 11000);
i = 0;
...
dieselacc.SendDynFloat32ArrayO1(slice32[:], tid, 0, DynMap[:], 21000);
